...
public class YahooFinanceService
{
    ...
    public async Task<List<QuoteItem>> GetQuotesAsync(IEnumerable<string> rawSymbols)
    {
        ...
        var resultMap = new Dictionary<string, QuoteItem>(StringComparer.OrdinalIgnoreCase);

        var bulkQuotes = await TryBulkQuoteAsync(normalized);
        foreach (var quote in bulkQuotes)
        {
        ...
        return OrderByInput(normalized, resultMap.Values.ToList());
    }
    ...
    private async Task<List<QuoteItem>> TryBulkQuoteAsync(List<string> normalized)
    {
        ...
        foreach (var url in urls)
        {
            try
            {
                ...
                {
                    AppDiagnostics.LogHttpFailure("YahooFinanceService", url, (int)response.StatusCode, response.ReasonPhrase ?? string.Empty, string.Empty);
                    continue;
                }
                ...
                foreach (var item in resultArray.EnumerateArray())
                {
                    ...
                    var previousClose = GetDecimal(item, "regularMarketPreviousClose");
                    if (previousClose == 0)
                    {
                        previousClose = GetDecimal(item, "previousClose");
                    }
            ...
            {
                AppDiagnostics.LogError("YahooFinanceService", ex);
            }
        }
    ...
    private async Task<QuoteItem?> TryChartQuoteAsync(string symbol)
    {
        var urls = new[]
        {
            $"https://query1.finance.yahoo.com/v8/finance/chart/{Uri.EscapeDataString(symbol)}?interval=1m&range=5d&includePrePost=false",
            $"https://query2.finance.yahoo.com/v8/finance/chart/{Uri.EscapeDataString(symbol)}?interval=1m&range=5d&includePrePost=false",
            $"https://query1.finance.yahoo.com/v8/finance/chart/{Uri.EscapeDataString(symbol)}?interval=5m&range=5d&includePrePost=false",
            $"https://query2.finance.yahoo.com/v8/finance/chart/{Uri.EscapeDataString(symbol)}?interval=5m&range=5d&includePrePost=false"
        };
        ...
        foreach (var url in urls)
        {
            try
            {
                ...
                var price = GetLastValidClose(item);
                if (price == 0)
                {
                    price = GetDecimal(meta, "regularMarketPrice");
                }

                var previousClose = GetDecimal(meta, "previousClose");
                if (previousClose == 0)
                {
                    previousClose = GetDecimal(meta, "chartPreviousClose");
                }
    ...
    private static decimal GetLastValidClose(JsonElement item)
    {
        ...
        for (var i = closeArray.GetArrayLength() - 1; i >= 0; i--)
        {
            var value = closeArray[i];
