using DMarket.Config;
using DMarket.Models;
using DMarket.Services;
...
public partial class MainWindow : System.Windows.Window
{
    ...
    public MainWindow()
    {
        ...
        AppDiagnostics.IsEnabled = _settings.DebugMode;
        ...
        Loaded += async (_, _) =>
        {
            AutoUpdater.MigrateLegacyExeNameIfNeeded();
    ...
    private WinForms.NotifyIcon CreateNotifyIcon()
    {
        ...
        var debugModeItem = new WinForms.ToolStripMenuItem("デバッグモード", null, (_, _) =>
        {
        ...
        var errorLogItem = new WinForms.ToolStripMenuItem("エラー確認", null, (_, _) => OpenErrorLog());
        ...
        menu.Opening += (_, _) => debugModeItem.Checked = _settings.DebugMode;
    ...
    private async Task RefreshQuotesAsync()
    {
        ...
        try
        {
            ...
            var allSymbols = _settings.Symbols
                .Concat(InternalPortfolio.Entries.Select(x => x.Symbol))
                .Where(x => !string.IsNullOrWhiteSpace(x))
                .Distinct(StringComparer.OrdinalIgnoreCase)
                .ToList();

            var quotes = await _yahooFinanceService.GetQuotesAsync(allSymbols);
            var quoteMap = quotes.ToDictionary(x => x.Symbol, StringComparer.OrdinalIgnoreCase);

            _rows.Clear();

            var fundRow = BuildPortfolioRow(quoteMap);
            if (fundRow is not null)
            {
                _rows.Add(fundRow);
            }
            ...
            {
                var normalizedSymbol = YahooFinanceService.NormalizeSymbol(symbol);
                if (!quoteMap.TryGetValue(normalizedSymbol, out var quote))
                {
                    continue;
                }

                _rows.Add(ToQuoteRow(quote));
            }
        ...
        catch (Exception ex)
        {
            ...
            AppDiagnostics.LogError("RefreshQuotesAsync", ex);
    ...
    private QuoteRow? BuildPortfolioRow(Dictionary<string, QuoteItem> quoteMap)
    {
        if (InternalPortfolio.Entries.Count == 0)
        {
            return null;
        }

        decimal totalValue = 0;
        decimal totalPreviousValue = 0;
        var missingSymbols = new List<string>();

        foreach (var entry in InternalPortfolio.Entries)
        {
            var normalizedSymbol = YahooFinanceService.NormalizeSymbol(entry.Symbol);
            if (!quoteMap.TryGetValue(normalizedSymbol, out var quote))
            {
            ...
            totalValue += quote.Price * entry.Quantity;
            var previousPrice = quote.PreviousClose > 0 ? quote.PreviousClose : quote.Price - quote.Change;
            totalPreviousValue += previousPrice * entry.Quantity;
        }
        ...
        if (totalValue == 0)
        {
            return new QuoteRow
            {
                DisplaySymbol = InternalPortfolio.FundCode,
                CompanyName = missingSymbols.Count > 0 ? $"{InternalPortfolio.FundName}(一部未取得)" : InternalPortfolio.FundName,
                PriceText = "-",
                ChangeText = "計算不可",
                ChangeBrush = Media.Brushes.OrangeRed
            };
        ...
        var fundPrice = totalValue / InternalPortfolio.DisplayDivisor;
        var fundPreviousPrice = totalPreviousValue / InternalPortfolio.DisplayDivisor;
        var fundChange = fundPrice - fundPreviousPrice;
        var fundChangePercent = fundPreviousPrice == 0 ? 0 : (fundChange / fundPreviousPrice) * 100m;

        return new QuoteRow
        {
            DisplaySymbol = InternalPortfolio.FundCode,
            CompanyName = missingSymbols.Count > 0 ? $"{InternalPortfolio.FundName}(一部未取得)" : InternalPortfolio.FundName,
            PriceText = fundPrice.ToString("0.##", CultureInfo.InvariantCulture),
            ChangeText = $"{fundChange:+0.##;-0.##;0} ({fundChangePercent:+0.##;-0.##;0}%)",
            ChangeBrush = fundChange > 0 ? Media.Brushes.LimeGreen : fundChange < 0 ? Media.Brushes.OrangeRed : Media.Brushes.White
        };
    }

    private static QuoteRow ToQuoteRow(QuoteItem quote)
    {
        return new QuoteRow
        {
            DisplaySymbol = quote.DisplaySymbol,
            CompanyName = quote.CompanyName,
            PriceText = quote.Price.ToString("0.##", CultureInfo.InvariantCulture),
            ChangeText = $"{quote.Change:+0.##;-0.##;0} ({quote.ChangePercent:+0.##;-0.##;0}%)",
            ChangeBrush = quote.Change > 0 ? Media.Brushes.LimeGreen : quote.Change < 0 ? Media.Brushes.OrangeRed : Media.Brushes.White
        };
    }
    ...
    {
        var window = new DMarket.Windows.ErrorLogWindow
        {
            Owner = this
        };
        window.ShowDialog();
